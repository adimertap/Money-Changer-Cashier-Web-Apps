...
<main class="mt-3">
    <div class="card bg-light mb-3">
        <div class="card-body p-3">
            <p class="fs--1 mb-0"> <span class="fas fa-exchange-alt me-2" data-fa-transform="rotate-90"></span>
                ...
                <input type="hidden" name="id_modal" id="id_modal" value="{{ $transaksi->id_modal }}">
            ...
            <div class="position-relative z-index-2">
                <div class="row">
                    <div class="col-8">
                        ...
                        <h3 class="text-primary mb-1 mt-2">Edit Transaksi Jual Valas!</h3>

                        <p>Edit Transaksi pada nomor ini</p>
                        <h6 class="text-primary">Nomor Order: #{{ $transaksi->kode_transaksi }}</h6>
                        <hr>
    ...
    <form action="{{ route('transaksi-jual.update', $transaksi->id_transaksi) }}" id="form" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="row gx-3">
            <div class="col-12">
                            ...
                            <table class="table table-hover table-striped overflow-hidden" id="dataTableKonfirmasi">
                                ...
                                <tbody id="konfirmasi" style="font-size: 15px!important">
                                    ...
                                    <tr id="item-{{ $item->id_detail_transaksi }}" role="row" class="odd">
                                        ...
                                        <td class="harga_currency_edit">Rp&nbsp;{{
                                            number_format($item->jumlah_currency) }}
                                        </td>
                                        <td class="jumlah_edit">{{ $item->jumlah_tukar }}</td>
                                        <td class="total_edit">Rp&nbsp;{{ number_format($item->total_tukar)}}</td>
                                        <td>
                ...
                <div class="card-body">
                    <div class="row">
                            ...
                            <div class="d-flex"><img class="me-3" src="../../falcon/assets/img/icons/shield.png" alt="" width="60" height="60">
                                <div class="flex-1">
                                    ...
                                    <div class="fs-4 mt-2 fw-semi-bold">All Total: <span class="text-primary">
                                            <span class="grand_total" id="grand_total">Rp&nbsp;
                        ...
                        <div class="col-md-5 col-xl-12 col-xxl-5 ps-lg-4 ps-xl-2 ps-xxl-5 text-center text-md-start text-xl-center text-xxl-start">
                            ...
                            <button class="btn btn-primary mt-3 px-4 py-2" data-bs-toggle="modal" data-bs-target="#error-modal" type="button">Update Data
                            </button>
...
<div class="modal fade" id="error-modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 500px">
        <div class="modal-content position-relative">
            ...
            <div class="modal-footer">
                <button class="btn btn-secondary" type="button" data-bs-dismiss="modal">Close</button>
                <button class="btn btn-success" type="button" id="btnSubmit" onclick="submitdata(event, {{ $transaksi->id_transaksi }})">Kirim dan Update </button>
            </div>
        </div>
    </div>
</div>
...
<script>
        ...
        if (check_grand == true) {
            ...
            for (let index = 0; index < detail.length; index++) {
                ...
                if (dataform2.length == 0) {
                    ...
                    var jumlah_modal_2 = jumlah_modal.replace(/[\.\,]/g, '')
                    var total_2 = total.replace(/[\.\,]/g, '')
                    var data = {
                        _token: _token
                        , kode_transaksi: kode_transaksi
                        , keterangan: keterangan
                        , id_modal: id_modal
                        , jumlah_modal: jumlah_modal_2
                        , total: total_2
                        , detail: dataform2
                    }
                    ...
                    $.ajax({
                        method: 'PUT',
                        url: '/transaksi-jual/' + id_transaksi,
                        data: data,
                        beforeSend: function () {
                        ...
                        success: function(response) {
                            ...
                            window.location.href = '/transaksi-jual'
    ...
    function hapusdata(element) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                var jumlah_trim = jumlah.split('Rp')[1].replace(/[\.\,]/g, '').trim();  // Remove dots and commas

                // GRAND TOTAL
                var grand_total = $('#grand_total').html();
                var grand_total_trim = grand_total.split('Rp&nbsp;')[1].replace(/[\.\,]/g, '').trim();  // Remove dots and commas
                ...
                // MODAL
                var jumlah_modal = $('#jumlah_modal').html();
                var jumlah_modal_trim = jumlah_modal.split('Rp&nbsp;')[1].replace(/[\.\,]/g, '').trim();  // Remove dots and commas
                console.log(jumlah_trim);  // For debugging
                console.log(jumlah_modal_trim);  // For debugging
                var jumlah_total_fix = parseInt(jumlah_modal_trim) - parseInt(jumlah_trim);
                console.log(jumlah_total_fix);  // For debugging
    ...
    $(document).ready(function() {
        ...
        $('select[name="id_currency"]').on('change', function() {
            ...
            if (id_currency) {
                $.ajax({
                    url: '/transaksi-jual/getkursJumlah/' + id_currency
                    , type: "GET"
                    , dataType: "json"
